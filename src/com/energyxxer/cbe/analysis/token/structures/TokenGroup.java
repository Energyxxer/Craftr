...
/**
 * Represents a group of token items. This represents a
 * structure a sequence of tokens should meet to match
 * a given token structure.
 * */
public class TokenGroup extends TokenItem {
	...
	public TokenMatchResponse match(List<Token> tokens) {
		if(optional) return new TokenMatchResponse(true, null, length());
		if(tokens.size() == 0 && !optional) return new TokenMatchResponse(false, null, 0);
		int currentToken = 0;
