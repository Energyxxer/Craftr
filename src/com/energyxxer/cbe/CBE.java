...
public class CBE {
	...
	public static void main(String[] args) {
		...
		sampleStream.add(new Token("phyg","null",0,0));
		sampleStream.add(new Token("base","null",0,0));
		sampleStream.add(new Token("pig","null",0,0));
		sampleStream.add(new Token("{...","null",0,0));*/
		
		File file = new File(Preferences.get("workspace_dir") + File.separator + "Jetpack Brawl.txt" + File.separator + "obsidian_boat.mcbe");

		sampleStream.add(new Token("[",file,1,1));
		sampleStream.add(new Token("1",TokenType.NUMBER,file,1,1));
		sampleStream.add(new Token(",",file,1,1));
		sampleStream.add(new Token("3.2",TokenType.NUMBER,file,1,1));
		sampleStream.add(new Token(",",file,1,1));
		sampleStream.add(new Token("92",TokenType.NUMBER,file,1,1));
		sampleStream.add(new Token(",",file,1,1));
		sampleStream.add(new Token("]",file,10,5));

		/*for(int i = 0; i < sampleStream.size(); i++) {
			System.out.print(sampleStream.get(i));
			if(i < sampleStream.size()-1) System.out.print(" ");
		}*/
