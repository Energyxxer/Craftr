...
public class ExplorerItemLabel extends JLabel implements MouseListener {
	...
	public void updateLabel() {
		...
		if (file.isDirectory()) {
			this.setIcon(new ImageIcon(ImageManager.load("/assets/icons/folder.png").getScaledInstance(16, 16,
			...
			try {
				...
				if (file.getParentFile().getCanonicalPath() == workspaceFile.getCanonicalPath()) {
					this.setIcon(new ImageIcon(ImageManager.load("/assets/icons/project.png").getScaledInstance(16, 16,
							java.awt.Image.SCALE_SMOOTH)));
				}
			} catch (IOException e) {
	...
	protected void paintComponent(Graphics g) {
		...
		if (selected) {
			...
			GradientPaint grad = new GradientPaint(this.getWidth() / 2, 0, Color.WHITE, this.getWidth() / 2,
					this.getHeight(), selected_background);
